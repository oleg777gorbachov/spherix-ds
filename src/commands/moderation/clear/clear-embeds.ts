...
const meta = new SlashCommandBuilder()
  .setName("clear-embeds")
  .setDescription("Clears embed messages")
...
export default command(meta, async ({ interaction }) => {
  ...
  const messages = allMessages.filter((e) => e.embeds.length > 0);
  ...
  if (size === 0) {
    return interaction.reply({
      ephemeral: true,
      content: `Can't find any embed messages!`,
    });
  }

  return interaction
    .reply({
      content: `Cleared ${size} embeds!`,
      fetchReply: true,
    })
    .then((message) =>
