...
const meta = new SlashCommandBuilder()
  .setName("automod-mention")
  .setDescription("Setups mention settings")
  .addBooleanOption((option) =>
    option
      .setName("state")
      .setDescription("Automod mention state")
      .setRequired(true)
  );
...
export default command(meta, async ({ interaction }) => {
  ...
  if (model) {
    ...
    await new automod({ gid, mention: state }).save();
  }
  ...
  return interaction.reply({
    embeds: [
      embed({
        title: `⚔️ Automod`,
        description: `Automoderation mention state - ${state}`,
        color: Colors.white,
      }),
    ],
  });
