...
const meta = new SlashCommandBuilder()
  ...
  .setDescription("Setups invites settings")
  .addBooleanOption((option) =>
    option
      .setName("state")
      .setDescription("Automod invites state")
      .setRequired(true)
  );
...
export default command(meta, async ({ interaction }) => {
  ...
  if (model) {
    ...
    await new automod({ gid, invites: state }).save();
  }
  ...
  return interaction.reply({
    embeds: [
      embed({
        title: `⚔️ Automod`,
        description: `Automoderation invites state - ${state}`,
        color: Colors.white,
      }),
    ],
  });
